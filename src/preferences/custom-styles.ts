...
export function addCustomCssDialogButton({
    window,
    group,
    settings,
}: {
...
function buildEnabledLabel({
    ...
    function updateEnabledLabel(): void {
...
function showDialog({
    ...
    function customStylesChanged(text: string): void {
...
function registerFailedToast({
    ...
    const showFailedToastConnection = settings.connect(
        `changed::custom-styles-failed`,
        showFailedToast,
    );
...
function buildHeaderBar({
    ...
    settings: Gio.Settings;
    showAppStyles: () => void;
    showCustomStyles: () => void;
}): { headerBar: Adw.HeaderBar; applyButton: Gtk.Button } {
...
function applicationStylesBox({ settings }: { settings: Gio.Settings }): Gtk.Widget {
    ...
    const descriptionLabel = new Gtk.Label({
        label:
            "The application styles that are generated based on the extension's appearance preferences. " +
...
function customStylesBox({
    settings,
    customStylesChanged,
}: {
    settings: Gio.Settings;
    customStylesChanged: (text: string) => void;
}): Gtk.Widget {
    const box = new Gtk.Box({
        orientation: Gtk.Orientation.VERTICAL,
        halign: Gtk.Align.FILL,
    ...
    textView.buffer.connect('changed', () => {
        const text =
            textView.buffer.get_text(
                textView.buffer.get_start_iter(),
                textView.buffer.get_end_iter(),
                false,
            ) ?? '';
